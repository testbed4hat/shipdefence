...
class SergeGame:
    ...
    def send_message(self, message: dict):
        ...
        response = requests.put(
            self.api_endpoint, json.dumps(message), headers={"Content-Type": "application/json"}
        )
        if not response.ok:
    ...
    def send_WA_message(self, channel: int, threat_id: str, weapon_id: int, threat_type: str, eta_minute: int, velocity: int,
                        target: int):
        msg_wa = MSG_WA.copy()
